...
bool Conversion::validateInfix(string Infix)
{
	if(Infix.length()<=2){
	...
	unsigned int index;							//declaring Iterator

	//checking for parenthesis
	Stack<char>* stack=new Stack<char>();
	for(index=0;index<Infix.length();index++)
	{
		if(Infix[index]=='('){					//pushing opening bracket in stack
			stack->push(Infix[index]);
		}
		else if(Infix[index]==')'){
			if(!stack->isEmpty())				//checking opening bracket for closing bracket
				stack->pop();
			else								//no match found
			{
				cout<<"Invalid Expression Entered, Incorrect Parenthesis Order\n";
				return false;
			}
		}
	}

	if(!stack->isEmpty())						//some brackets are not closed
	{
		cout<<"Invalid Expression Entered, Incorrect Parenthesis Order\n";
		return false;
	}

	//checking for the number of operands vs operators
	int numOperands=0,numOperators=0;			//counters to check for number of operators and operands

	for(index=0;index<(Infix.length());index++){
		if(isalpha(Infix[index]))				//if operand then increase its counter
			numOperands+=1;
												//if operand
		if(Infix[index]=='+' or Infix[index]=='-' or Infix[index]=='*' or Infix[index]=='/' or Infix[index]=='%' or Infix[index]=='^'){
			if(numOperands==0) {
				cout<<"Invalid Expression, Operator Error\n";
				return false;
			}
			else if((isalpha(Infix[index-1]) or Infix[index-1]==')') and (Infix[index+1]=='(' or isalpha(Infix[index+1])))
				numOperators+=1;
			else {
				cout<<"Invalid Expression, Operator Error\n";
				return false;
	...
	if(numOperands!=(numOperators+1))			//num of operators should be 1 less than operands
		return false;

	return true;								//no glitch found
}

void Conversion::Infix_to_Postfix(string Infix)
{
	if(!validateInfix(Infix))					//validating Infix Expression
		return;
...
double Conversion::prefixEvaluation(string prefix) {
	...
	while(index>=0)
	{
		...
		{
			cout<<"Enter the value of "<<prefix[index]<<" : ";
			cin>>val;
			stack.push(val);
		}
		else
